...
class NewScoreView:
    ...
    def render(self):
        rect = pygame.Rect(self._width//2 - 300, self._height//2 - 200, 600, 400)
        ...
        rect = text.get_rect()
        self._screen.blit(text, ((self._width - rect[2])//2,self._height//2 - 150))
        ...
        while True:
            name_rect = pygame.Rect(self._width//2 - 100, self._height//2 - 15, 200, 30)
            pygame.draw.rect(self._screen, self._colors["White"], name_rect)

            player_input = self._event_handler.handle_name_input(text_content, self._event_queue)
